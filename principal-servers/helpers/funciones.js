  ...
  if(req.body != undefined){
    ...
    db.serialize(function (){
         ...
         db.each(`SELECT * FROM usuarios`, (error, row) => {
          ...
          let resposta = {resposta:"intdocidos correctamente"}
          res.send(resposta)
        });//db.each
    });//db.serialize
...
const isLogueado = function(req,res){
  
console.log('está en isLogueado : ');
  
  db.get(`SELECT * FROM usuarios where nombre_cli = ?`,[`${req.body.nomeUsuario}`],(error,row)=>{
    if (error) {
            throw new Error(error.message);
          }
